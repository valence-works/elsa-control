...
Challenge redemption records the connector identity and completes pairing, but it leaves lifecycle status `Pending`. #482 owns authenticated heartbeat, release evidence, capabilities, a signed candidate Studio destination, and the transition to `Connected` or `Degraded`; customer approval and presentation of that destination stay in #483. Disconnect revokes connector proof access immediately and preserves the revoked connection plus value-free append-only audit. Generic deployment command claims still use their existing engine authorization and remain outside this connection's authority until #484 integrates each permitted action.

Issue #482 adds `POST /api/runtime/external-engine-connections/{connectionId}/heartbeat`. The enrolled connector signs operation `external-engine.heartbeat.submit` and a server-defined canonical digest over the complete normalized report. Control consumes the proof nonce durably, requires a strictly increasing sequence, limits accepted cadence, and commits the observation only while the same connector identity remains active and unrevoked. A rate-limited response includes `Retry-After` plus `retryAfterSeconds`; because the rejected proof nonce has already been consumed, the connector must sign the retry with a fresh nonce. Protocol `1` accepts the read-only `connection.status` and `studio.open` capabilities; unknown capabilities or protocol versions reject the whole observation and never grant deployment or runtime-control authority.

For protocol `1`, the signed payload is SHA-256 over compact UTF-8 JSON without a BOM. Properties appear in this exact order: `sequence`, `observedAt`, `connectorProtocol`, `connectorVersion`, `runtimeHealth`, `runtimeKind`, `observedDistribution`, `observedVersion`, `studioDestination`, `capabilities`, `components`. The UTC observation time uses `yyyy-MM-dd'T'HH:mm:ss.fffffff'Z'`; runtime health is `unknown`, `healthy`, or `unhealthy`; absent optional strings are explicit JSON `null`; capabilities are distinct and ordinally sorted; components are sorted by case-insensitive ID then digest and use the ordered properties `id`, `imageDigest`. Strings use standard JSON escaping. The SHA-256 bytes are encoded as unpadded base64url. The implementation carries a fixed interoperability vector so another connector implementation can verify its encoder.

Control records its own receipt time as `last authenticated` and projects a connection stale after 90 seconds without a newly accepted heartbeat. Staleness changes the read projection to `Degraded` and `Unreachable` while retaining the last safe facts. A later accepted heartbeat recovers the projection. The connector observation time is retained for diagnostics and ordering evidence, but does not control freshness.

Release labels remain server-computed. An observed distribution/version without exact immutable component evidence stays `SelfReported`. `VerifiedManifest` means the paired connector reported the complete component digest set matching an admitted signed `valence-runtime` catalog manifest; it is not independent host attestation. `SupportedRelease` additionally requires an explicitly allowlisted Elsa OSS distribution, exact component match, and a Control-owned catalog lifecycle of `Supported`. The safe projection stores the matched manifest digest, never the raw heartbeat, proof, nonce, signature, workflow payload, path, credential, or private diagnostic.

The connector may report a candidate Studio destination as part of its signed observation, but #482 does not approve or expose that URL. A later customer-facing flow must show the destination host and require an authorized customer confirmation before Control stores an openable Studio destination. This keeps a compromised connector from silently turning the dashboard action into an arbitrary outbound navigation.
