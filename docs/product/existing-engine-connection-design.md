...
1. **Security: define and implement runtime-originated engine enrollment** — one-time scoped challenge, proof-of-possession, replay/expiry/revocation, tenant/workspace isolation, and threat model. Gate: protocol review and tests for replay, cross-workspace redemption, stolen/expired challenge, rotation, and revoke.
2. **Control: add external connection persistence and narrow customer/runtime APIs** — no browser-set capability authority; safe audit projection; exact `AllowCloudBff` routes; `Read`/`ManageSetup` authorization. Gate: API security tests, idempotent pairing, secret/log redaction, and proof that BFF tokens cannot call generic deployment or admin routes.
3. **Runtime: emit authenticated heartbeat and release/capability evidence** — signed/allowlisted facts from the runtime/connector, with unsupported versions represented explicitly. Gate: forged/self-reported version cannot claim a governed release or elevate capabilities.
4. **Cloud: add connect, status, repair, disconnect, and external Studio UX** — accessible pending/failure/stale states, no provisioning language, and explicit customer-ownership copy. Gate: browser tests against pending, success, stale, revoked, unsupported, and retry responses; verify no secret in browser persistence.
5. **Optional deployment/control capability slice** — only after the connection trust work, align each action with existing deployment permissions and runtime command audit. Gate: separate tests for `ExecuteDeployment` and `ExecuteControls`, deny unadvertised operations, and compatibility checks; no lifecycle delete/upgrade claim without its own contract.
6. **Live connection rehearsal and support runbook** — public and private-network engine cases, connector rotation/revocation, lost connectivity, and disconnect behavior. Gate: evidence shows no Azure resource is created or changed by connect/disconnect and Studio remains customer-authenticated.
