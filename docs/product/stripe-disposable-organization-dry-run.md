...
T0/T1 may report **gate-allow create** only from a build containing #438: Stripe
`Trial` and `Active` projection must have set `ManagedHostingEnabled=true` on
the entitlement snapshot. Earlier builds are expected to fail that gate even
after a successful trial-to-paid transition.
