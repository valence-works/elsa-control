...
class AzureApiDeployWorkflowTests(unittest.TestCase):
    ...
    def test_deploy_and_rollback_use_the_captured_runtime_mode(self) -> None:
        ...
        main_guard_start = self.source.index("      - name: Require an approved ref for Azure mutation")
        ...
        self.assertIn('refs/heads/candidate/staging', main_guard)
        self.assertIn('[ "$TARGET_ENVIRONMENT" = "test" ]', main_guard)
        self.assertIn('[ "$DEPLOY_MODE" = "build" ]', main_guard)
        self.assertIn('[ "$DEPLOY_MODE" = "promote" ]', main_guard)
